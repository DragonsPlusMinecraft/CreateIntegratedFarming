...
public class CottonHarvestBehaviour implements CustomHarvestBehaviour {
    @Override
    ...
    private static @Nullable BlockState getMatureState(Level level, BlockPos pos) {
        ...
        if (!state.is(HHModBlocks.COTTON_CROP.get()) || state.getValue(CropBlock.AGE) != 7)
            return null;
        return state;
    }

    private static ItemStack createDrop(Level level) {
        return new ItemStack(HHModItems.COTTON.get(), 1 + level.random.nextInt(2));
    }

    private static void reset(Level level, BlockPos pos, BlockState state) {
        level.playSound(
                null, pos, SoundEvents.CROP_BREAK, SoundSource.BLOCKS,
                1.0F, 0.8F + level.random.nextFloat() * 0.4F);
        level.setBlock(pos, state.setValue(CropBlock.AGE, 5), 2);
    }
}
